...
def rank():
    ...
    folder_path = "../loaded_influencer_data"  
    ...
    for name in influencer_list:
        tmp=pd.DataFrame(index=[0],columns=['profile','influencer','total_ER','total_view',
                                            ...
                                            "Non-fixed_comment_ER","Non-fixed_saved_ER"])
        try:
            profile=pd.read_excel(folder_path+'/'+name+'/'+name+'.xlsx', usecols = "A,B,C,D")
            video=pd.read_excel(folder_path+'/'+name+'/'+name+'_video.xlsx', usecols = "A,B,C,D,E,F,G,H,I,J,K,L,M")         
        except:
            continue 
    ...
    if type_options=="Tag":
        ...
        for i, row in enumerate(sorted(tag_fig[selected_tag].items(), key=lambda x: x[1],reverse=True)):
            profile="../loaded_influencer_data/"+row[0]+"/profile.jpg"    
            influencer=row[0]
            if i==0:
                rank='🥇 1 st '
            elif i==1:
